...
function activeItem(index: number) {
  const scenInfo = document.getElementById("scene")?.getBoundingClientRect()
  const rectInfo = document.getElementById("photo_" + index)?.getBoundingClientRect()
  if (!rectInfo || !containerWidth || !containerHeight || !scenInfo) return
  // 先关闭所有激活的item
  unactiveItem()
  ...
  const newWidth = itemInitWidth * 3
  const newHeight = itemInitWidth * 3

  item.active_width = newWidth
  item.active_height = newHeight

  console.log(scenInfo)

  // 中心放大
  let trans_x = (item.active_width - itemInitWidth) / 2
  let trans_y = (item.active_height - itemInitHeight) / 2
  // if (rectInfo.left - trans_x  < 0) {
  //   trans_x = 0 - rectInfo.left
  // }
  // if (rectInfo.top - trans_y - scenInfo.top < 0) {
  //   trans_y = rectInfo.top - scenInfo.top
  // }
  // if (rectInfo.left + rectInfo.width + trans_x > containerWidth) {
  //   trans_x =  trans_x+ rectInfo.width
  // }
  // if (rectInfo.top + rectInfo.height + trans_y > containerHeight) {
  //   trans_y = trans_y+ rectInfo.height
  // }

  item.zIndex = 9
  // item.transform = `translate(${trans_x}px, ${trans_y}px)`
  item.active_left = -trans_x
  item.active_top = -trans_y

  nextTick(() => {
    let activeRectInfo = document.querySelector("#photo_" + index + " .photo-active")?.getBoundingClientRect()
    if (!activeRectInfo) return

    if (activeRectInfo.left < scenInfo.left) {
      item.active_left = item.active_left + scenInfo.left - activeRectInfo.left
    }
    if (activeRectInfo.top < scenInfo.top) {
      item.active_top = item.active_top + scenInfo.top - activeRectInfo.top
    }

    if (activeRectInfo.left + activeRectInfo.width > scenInfo.right) {
      item.active_left = item.active_left - (activeRectInfo.left + activeRectInfo.width - scenInfo.right)
    }

    if (activeRectInfo.top + activeRectInfo.height > scenInfo.bottom) {
      item.active_top = item.active_top - (activeRectInfo.top + activeRectInfo.height - scenInfo.bottom)
    }

    nextTick(() => {
      activeRectInfo = document.querySelector("#photo_" + index + " .photo-active")?.getBoundingClientRect()
      if (!activeRectInfo) return

      // item中心点坐标
      const item_center = [activeRectInfo?.left + activeRectInfo?.width / 2, activeRectInfo?.top + activeRectInfo?.height / 2]
...
function triggerList(item: any, index: number, item_center: any[]) {
  ...
  photoList.value.forEach((item1: any, index1: number) => {
    ...
    // item的放大系数
    const trans_x = (item.active_width - itemInitWidth) * itemInitWidth / distance_n / radius
    const trans_y = (item.active_height - itemInitHeight) * itemInitHeight / distance_n / radius
